...
startupinfo = sp.STARTUPINFO() if hasattr(sp, "STARTUPINFO") else None
if startupinfo is not None:
    startupinfo.dwFlags = sp.CREATE_NEW_CONSOLE | sp.STARTF_USESHOWWINDOW if hasattr(sp, "STARTUPINFO") else None
...
def check_output(*popenargs, **kwargs):
    ...
    if retcode:
        ...
        if cmd is None:
            cmd = popenargs[0]
        error = sp.CalledProcessError(retcode, cmd, output)
        error.output = output
        raise error
    return output
