...
from Utils.utils import AnytimeFpsIndexer
...
class VideoReaderAbstract(object):
    ...
    def __init__(self, filename, inputdict=None, outputdict=None, outputfps=-1, inputfps=-1, verbosity=0):
    ...
    def _read_frame_data(self):
        ...
        try:
            ...
            if len(arr) != framesize:
                if self.verbosity > 0:
                    print(self._proc.stderr.read(600), file=sys.stderr)  # read from stderr to obtain error
                return np.array([])
...
class VideoWriterAbstract(object):
    ...
    def _warmStart(self, M, N, C, dtype):
        ...
        if ("-s" in self.inputdict):  # TODO do we put an invalid output resolution of pic when missing "-s"?
